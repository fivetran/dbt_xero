...
first_contact as (
    ...
    select 
        ...
        coalesce(
            ...
            -- inculde bank transaction contact if the user is leveraging this source table
            {% if var('xero__using_bank_transaction', True) %}
                , bank_transactions.contact_id
            {% endif %}

            -- inculde credit note contact if the user is leveraging this source table
            {% if var('xero__using_credit_note', True) %}
    ...
    -- join bank transaction contact if the user is leveraging this source table
    {% if var('xero__using_bank_transaction', True) %}
    ...
    -- join credit note contact if the user is leveraging this source table
    {% if var('xero__using_credit_note', True) %}
