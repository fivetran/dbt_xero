...
{{ adapter.dispatch('partition_by_source_relation', 'xero') (has_other_partitions, alias) }}
...
{% macro default__partition_by_source_relation(has_other_partitions='yes', alias=None) -%}
    ...
    {# Prioritizes xero_sources -> union_schemas -> union_databases -> [] #}
    {% set union_variable = var('xero_sources', var('union_schemas', var('union_databases', []))) %}
